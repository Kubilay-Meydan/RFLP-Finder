    ...
    try:
        with open(snp_file, mode='r') as csvfile:
            ...
            for row in reader:
                ...
                left_primer_seq, right_primer_seq = design_primers(seq1, primer_params)
                if left_primer_seq is None or right_primer_seq is None:
                    print(f"Warning: Could not design primers for SNP at Chromosome {row['Chromosome']}, Position {row['Position']}")
                    continue
