...
def read_snp_file(snp_file_path):
    ...
    with open(snp_file_path, "r") as snp_file:
        for line in snp_file:
            ...
            nucleotide2 = parts[4]
            snp_data.append(
                (chromosome, position1, nucleotide1, nucleotide2)
            )
...
def extract_sequence_from_chromosome(
    genome_fasta_folder, snp_info, sequence_size
):
    chromosome, position1, nucleotide1, nucleotide2 = snp_info
    fasta_files = [
        f
        for f in os.listdir(genome_fasta_folder)
        if f.endswith(".fa")
        and get_chromosome_from_filename(f) == chromosome
    ]

    if not fasta_files:
        print(
            f"No FASTA file found for chromosome {chromosome}. Skipping."
        )
        return None

    fasta_file_path = os.path.join(
        genome_fasta_folder, fasta_files[0]
    )
    ...
    try:
        with open(fasta_file_path, "r") as fasta_file:
            ...
            if end > len(sequence):
                print(
                    f"Position {position1} is out of bounds for chromosome {chromosome}. Skipping."
                )
                return None
            ...
            if (
                position1 - start - 1 >= 0
                and position1 - start - 1 < len(sequence2)
            ):
        ...
        print(
            f"Error processing chromosome {chromosome} at position {position1}: {e}"
        )
        return None
...
def process_snp_data(genome_fasta_folder, snp_data, sequence_size):
    with mp.Pool(mp.cpu_count()) as pool:
        func = partial(
            extract_sequence_from_chromosome,
            genome_fasta_folder,
            sequence_size=sequence_size,
        )
        results = list(
            tqdm(pool.imap(func, snp_data), total=len(snp_data))
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract sequences around SNP positions."
    )
    parser.add_argument(
        "-gf",
        "--genome_fasta_folder",
        required=True,
        help="Path to the folder containing the genome FASTA files.",
    )
    parser.add_argument(
        "-s",
        "--snp_file",
    ...
    parser.add_argument(
        ...
        type=int,
        default=501,
        help="Number of nucleotides before and after the SNP position (default: 501).",
    )
    ...
    extracted_data = process_snp_data(
        genome_fasta_folder, snp_data, sequence_size
    )
