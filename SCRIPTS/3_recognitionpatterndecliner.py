...
def generate_iupac_possibilities(sequence):
    ...
    possibilities = [
        ...
        for p in itertools.product(
            *(iupac_dict[base] for base in sequence)
        )
    ]
...
def process_enzyme_sites(input_file, output_file):
    ...
    processed_sequences = (
        set()
    )  # Track (enzyme, sequence) to allow duplicates with different enzymes
    ...
    for index, row in df.iterrows():
        ...
        if "/" not in cut_sequence:
            print(
                f"Warning: Enzyme '{enzyme}' skipped because its Cut Sequence does not contain '/'."
            )
            continue
        ...
        possibilities = generate_iupac_possibilities(
        ...
        for seq in possibilities:
            ...
            if (enzyme, seq) in processed_sequences or (
                enzyme,
                rc_seq,
            ) in processed_sequences:
                continue
            ...
            if seq != rc_seq:
                reverse_cut_pos_value = (
                    seq_size - cut_pos
                )  # Cut position on the reverse complement
                results.append(
                    {
                        "Enzyme": enzyme,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process enzyme recognition sites from an input TSV file."
    )
    parser.add_argument(
        "-input_file", help="Path to the input TSV file."
    )
    parser.add_argument(
        "-output_file", help="Path to the output TSV file."
    )
