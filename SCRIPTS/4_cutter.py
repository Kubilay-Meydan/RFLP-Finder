...
def extract_amplicon(sequence, left_primer_seq, right_primer_seq):
    ...
    right_primer_seq_rc = str(
        Seq(right_primer_seq).reverse_complement()
    )
        ...
        with open(snp_file, mode="r") as csvfile:
            ...
            for row in reader:
                ...
                (
                    ...
                    right_primer_seq,
                    primer_explanation,
                ) = design_primers(seq1, primer_params)

                if (
                    left_primer_seq is None
                    or right_primer_seq is None
                ):
                ...
                amplicon_seq1 = extract_amplicon(
                    seq1, left_primer_seq, right_primer_seq
                )
                amplicon_seq2 = extract_amplicon(
                    seq2, left_primer_seq, right_primer_seq
                )

                if amplicon_seq1 is None or amplicon_seq2 is None:
                    print(
                        f"Warning: Primers do not anneal properly in one of the sequences at Chromosome {row['Chromosome']}, Position {row['Position']}"
                    )
                    continue
                ...
                if len(amplicon_seq1) != len(amplicon_seq2):
                    print(
                        f"Warning: Amplicon sizes differ for SNP at Chromosome {row['Chromosome']}, Position {row['Position']}"
                    )
                    continue
                ...
                for enzyme, recognition_seqs in enzyme_dict.items():
                    cuts_seq1 = find_cuts_with_sequences(
                        amplicon_seq1, recognition_seqs
                    )
                    cuts_seq2 = find_cuts_with_sequences(
                        amplicon_seq2, recognition_seqs
                    )

                    # Get positions as sets for set operations
                    positions_seq1_set = set(
                        cut["Position"] for cut in cuts_seq1
                    )
                    positions_seq2_set = set(
                        cut["Position"] for cut in cuts_seq2
                    )
                    positions_seq1 = sorted(positions_seq1_set)
                    positions_seq2 = sorted(positions_seq2_set)

                    # Compute fragment sizes
                    fragment_sizes_seq1 = compute_fragment_sizes(
                        amplicon_size, positions_seq1
                    )
                    fragment_sizes_seq2 = compute_fragment_sizes(
                        amplicon_size, positions_seq2
                    )

                    # Convert cut positions to strings
                    cuts_positions_seq1_str = ";".join(
                        map(str, positions_seq1)
                    )
                    cuts_positions_seq2_str = ";".join(
                        map(str, positions_seq2)
                    )
                    ...
                    cut_difference = abs(
                        num_cuts_seq1 - num_cuts_seq2
                    )

                    if cut_difference == 1:
                        # Find unique cuts
                        unique_cuts_positions = (
                            positions_seq1_set.symmetric_difference(
                                positions_seq2_set
                            )
                        )
                        ...
                        for pos in unique_cuts_positions:
                            if pos in positions_seq1_set:
                                ...
                                cut_seq = next(
                                    cut["RecognitionSequence"]
                                    for cut in cuts_seq1
                                    if cut["Position"] == pos
                                )
                                corresponding_seq = amplicon_seq2[
                                    pos - 1 : pos - 1 + len(cut_seq)
                                ]
                            else:
                                ...
                                cut_seq = next(
                                    cut["RecognitionSequence"]
                                    for cut in cuts_seq2
                                    if cut["Position"] == pos
                                )
                                corresponding_seq = amplicon_seq1[
                                    pos - 1 : pos - 1 + len(cut_seq)
                                ]
                            ...
                            corresponding_seq = (
                                corresponding_seq.upper()
                            )

                            unique_cuts_details.append(
                                f"Position: {pos}, {cut_amplicon} Cut: {cut_seq}, Other Amplicon Sequence: {corresponding_seq}"
                            )

                        # Combine unique cuts details into a single string
                        unique_cuts_str = " | ".join(
                            unique_cuts_details
                        )
                        ...
                        results.append(
                            {
                                ...
                                "Fragment Sizes Amplicon 1": ";".join(
                                    map(str, fragment_sizes_seq1)
                                ),
                                "Fragment Sizes Amplicon 2": ";".join(
                                    map(str, fragment_sizes_seq2)
                                ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Perform RFLP analysis on hypothetical amplicons generated by Primer3."
    )
    parser.add_argument("snp_file", help="Path to the SNP CSV file.")
    parser.add_argument(
        "enzyme_file", help="Path to the enzyme data file."
    )
    parser.add_argument(
        "-o",
        "--output",
        default="enzyme_cut_amplicons.csv",
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="Optimal primer melting temperature (Tm).",
    )
    parser.add_argument(
        "--PRIMER_MIN_TM",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="100-300",
        help='Desired PCR product size range (e.g., "100-300").',
    )
    ...
    primer_params = {
        ...
        "PRIMER_PRODUCT_SIZE_RANGE": [
            [
                int(x)
                for x in args.PRIMER_PRODUCT_SIZE_RANGE.split("-")
            ]
        ],
    }
    ...
    # Process SNP data
    results = compare_enzyme_cuts(
        args.snp_file, enzyme_dict, primer_params
    )
    ...
    try:
        with open(args.output, mode="w", newline="") as csvfile:
            fieldnames = [
                ...
                "Cuts in Amplicon 1",
                "Cuts in Amplicon 2",
                "Positions in Amplicon 1",
                "Positions in Amplicon 2",
                "Fragment Sizes Amplicon 1",
                "Fragment Sizes Amplicon 2",
                "Unique Cuts",
            ]
